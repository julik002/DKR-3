...
function per(x:real):real;
  ...
  begin
    repeat
      ...
      readln(t);
      case t of
        1:begin println('Функция:power(x,3)+(-2)*power(x,2)+(-5)*x+(8)'); readkey; end;
        2:begin println('Первообразная функции:(power(x,4))/4-2*(power(x,3))/3-5*((power(x,2))/2)+8*x'); readkey; end;
